...
const getAllUsers = async () => {
    const { rows } = await db.query("SELECT id, name, email, phone, is_premium, created_at FROM users WHERE role = 'user' ORDER BY created_at DESC");
    return rows;
};

const getRiskStatistics = async () => {
    // We will aggregate stats specifically by PCOS Risk Category for the primary dashboard chart
    const { rows } = await db.query('SELECT pcos_category as risk_category, COUNT(*) as count FROM assessments GROUP BY pcos_category');
    return rows;
};
